...
import com.feiche.driver.socket_test.util.ByteUtil;
import com.feiche.driver.socket_test.util.NetworkUtil;
        ...
        public void run() {
            ...
            try {
                while (state != STATE_CLOSE && state == STATE_CONNECT_SUCCESS && null != inStream) {
                    Log.v(TAG, "Rec :---------");
                    byte[] rec = new byte[8];
                    Log.v(TAG, "Rec :before read");
                    inStream.read(rec, 0, 8);
                    Log.v(TAG, "Rec :read");
                    int len = ByteUtil.getInt(rec, 0);
                    int type = ByteUtil.getInt(rec, 4);
                    byte[] str = new byte[len];
                    inStream.read(str, 0, len);
                    Log.v(TAG, "Rec :-----read2");
                    String recs = new String(str, "utf-8").trim();
                    if (null != respListener) {
                        respListener.onSocketResponse("len=" + len + "type=" + type + recs);
