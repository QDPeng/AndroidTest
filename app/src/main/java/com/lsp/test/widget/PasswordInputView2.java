...
public class PasswordInputView2 extends EditText {
    ...
    public PasswordInputView2(Context context, AttributeSet attrs) {
        ...
        final int defaultPasswordLength = 5;
        ...
        try {
            ...
            borderWidth = a.getDimension(R.styleable.PasswordInputView_bordersWidth, defaultBorderWidth);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //文字
        passwordPaint.setTextSize(20);
        float cx, cy = height / 2;
        float half = width / passwordLength / 2;
        String text = getText().toString().trim();
        float textSize = passwordPaint.getTextSize() / 2;


        for (int i = 0; i < textLength; i++) {
            cx = width * i / passwordLength + half;
            canvas.drawText(text.toCharArray(), i, 1, cx, cy , passwordPaint);
        }
